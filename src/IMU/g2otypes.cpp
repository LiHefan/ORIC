...
void EdgeNavStatePRV::computeError()
{
    ...
    // terms need to compute error in vertex i. except for bias error
    ...
    // terms need to compute error in vertex j, except for bias error
    ...
    const Vector3d dPij = M.getDeltaP();    // Delta Position pre-integartion measurement
...
void EdgeNavStatePRPointXYZ::linearizeOplus()
{
    ...
    // Jacobian of error w.r.t Pwb
    Matrix<double,2,3> JdPwb = -Jpi*(-Rcb*Rwb.transpose());
    // Jacobian of error w.r.t Rwb
...
bool EdgeGyrBias::read(std::istream& is) {return true;}
bool EdgeGyrBias::write(std::ostream& os) const {return true;}
