...
void KeyFrame::AddMapCuboid(MapCuboid* pMC, const size_t &idx)
{
    unique_lock<mutex> lock(mMutexFeatures);
...
{
    unique_lock<mutex> lock(mMutexFeatures);
    mvpMapCuboids[idx]=static_cast<MapCuboid*>(NULL);
}

void KeyFrame::EraseMapCuboidMatch(MapCuboid* pMC)
{
    int idx=pMC->GetIndexInKeyFrame(this);
    if(idx>=0)
        mvpMapCuboids[idx]=static_cast<MapCuboid*>(NULL);
}
