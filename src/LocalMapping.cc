...
void LocalMapping::KeyFrameCulling()
{
    ...
    for(vector<KeyFrame*>::iterator vit=vpLocalKeyFrames.begin(), vend=vpLocalKeyFrames.end(); vit!=vend; vit++)
    {
        ...
        if(pKF->mnId==0)
            continue;
        // Do not cull the oldest KF in LocalWindow
        // And the KF before this KF
        if(pKF == pOldestLocalKF || pKF == pPrevLocalKF)
            continue;
        // Check time between Prev/Next Keyframe, if langer than 0.5s(for local)/3s(others), do not cull
        ...
        // Do not drop KF before current KF
            ...
            if(pMP)
            {
                if(!pMP->isBad())
                {
                    ...
                    if(pMP->Observations()>thObs)
                    {
                        ...
                        const mapMapPointObs observations = pMP->GetObservations();
                        int nObs=0;
                        for(/*map<KeyFrame*, size_t>*/mapMapPointObs::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
                        {
